...
wm<-ics_map%>%filter( NHSER21NM=='Midlands')
...
df[1:5,]%>%kbl(caption="Ward map co-ordinates")%>%
  kable_styling()
...
building_map<-ward_map%>%filter(estate=='building'| str_detect(estate,'room'))

bed_map<-ward_map%>%filter( str_detect(estate,'bed'))
...
# produce plot
ggplot() +
  geom_sf(data = building_map,colour="black", fill=NA)+  #fill=NA sets background to no fill
  geom_sf(data = bed_map, aes(fill = value))+
  geom_label_repel(data = ward_map, aes(x = labelX, y = labelY, label = estate)) +
  theme_void()
...
wheelchair_img <-wheelchair%>% mutate(image = sample(c("wheelchair_alt.png"),size=1,
                             replace = TRUE))


ggplot() +
  geom_sf(data = building_map,colour="black", fill=NA)+
  geom_image(data=wheelchair_img,aes(x=x,y=y,image=image),size=0.1)+
  geom_sf(data = bed_map, aes(fill = value))+
